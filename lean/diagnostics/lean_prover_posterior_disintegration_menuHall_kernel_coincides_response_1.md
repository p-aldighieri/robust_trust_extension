...
========
ROLE: user (id=94eb94f9-3d09-40c3-870e-bd95910309f1)
========
...
========
ROLE: assistant (id=b1200e8a-5c46-4e56-aa4e-1e3718a50dd0)
========
lean_proof
target_lemma_slug: posterior_disintegration_menuHall_kernel_coincides
status: PROVED
tactics_used: [classical, have, exact, simpa, trans, filter_upwards, calc, rw, cases]
proof_length_lines: 47
introduces_have_clauses: 8
lean
theorem posterior_disintegration_menuHall_kernel_coincides
    (model : RobustTrustModel)
    (pd : PosteriorDisintegration model)
  ...
  have hbase :
    ...
    exact mh.q_eq_gamma_second.symm.trans mh.q_eq_qκ
  ...
  have hcomp :
      (MixtureMessageLaw model κ).compProd (pd.sourceLawβ κ) =
        (MixtureMessageLaw model κ).compProd (pd.sourceLawγα κ) := by
    exact (pd.sourceLawβ_disintegrates κ).symm.trans hγ_dis
  ...
  have hBeliefAsProfile_inj :
      ∀ {x y : Belief model.Ω},
        beliefAsProfile x = beliefAsProfile y → x = y := by
    intro x y hxy
    cases x
    cases y
    simpa [beliefAsProfile] using hxy

  filter_upwards [hsource, hb, hg] with m hsource_m hb_m hg_m
  ...
  have hprof :
    ...
    calc
      beliefAsProfile (pd.Pβ κ m)
          = beliefBarycenter ((pd.sourceLawβ κ) m) := hb_m.symm
      _ = beliefBarycenter ((pd.sourceLawγα κ) m) := by
            rw [hsource_m]
      _ = beliefAsProfile (pd.Pγα κ m) := hg_m

  exact hBeliefAsProfile_inj hprof
