...
========
ROLE: user (id=4f33afe6-3050-4dbc-bd2e-9fcc8a27b2c2)
========
...
========
ROLE: assistant (id=7adc7e6b-e09c-4a79-b282-46ac0a58115a)
========
review_control
verdict: PATCH_BIG
ready_for_formalization: false
recommended_next_phase: DEP_AUDIT_PATCHER
needs_inventory_lean_update: true
axiom_dependent_candidates: []
semantics_mismatch_count: 6
Verdict

VERDICT: PATCH_BIG

Reason: the kernel-side Fubini correction is substantively fixed in dep_audit.md, but several of the seven refined INVENTORY stubs are still not formalization-safe. The patch removed the visible True placeholders, but in multiple places it replaced them with theorem statements that are either too strong for their hypotheses, too strong for the source, or assume the desired conclusion under a new name. That is a bigger problem than a cosmetic stub cleanup, because the formalizer would either fail to instantiate the lemmas at the intended use sites or import false general-purpose theorem shapes as project facts.

Opinion and Next Move

Do one targeted inventory-statement patch before sending this to the Lean formalizer. The kernel row is almost ready; the stubs need another pass with use-site-shaped hypotheses rather than generic theorem-shaped wishes.

Detailed Review
Pass-1 Issues Tracker

Kernel-side Fubini broadening: addressed, with one document-sync caveat.

MeasureTheory.Measure.integral_compProd is the right primary orientation for the use sites that build the joint source-message law as τ.compProd β. It gives the Bochner Fubini identity at the measure level, and the stated route through Measure.integral_compProd plus Kernel.compProd_apply is exactly the right workaround given that ProbabilityTheory.Kernel.integral_compProd does not exist in Lean 4.29.0.

The companion names are also correctly useful: Measure.lintegral_compProd for Tonelli/nonnegative integrals, Kernel.lintegral_compProd for kernel-level nonnegative composition, Measure.compProd and Kernel.compProd for the constructions. Ordinary MeasureTheory.integral_prod is properly downgraded to fallback status.

Caveat: dep_audit_proposed.md still lists MeasureTheory.integral_prod as Candidate 1 in the Fubini row and does not visibly list MeasureTheory.Measure.integral_compProd as the primary candidate. dep_audit.md is right; the proposal file is stale and should be synchronized so the formalizer does not follow the old product-measure path by accident.

Seven placeholder stubs refined: partially addressed overall, but not enough for formalization.

Per-stub status:

profile_geometry_import: partially addressed.
The patch fixes the previous type-shape problem by making W := Set.range Φ ⊆ Ω → ℝ, and the nonempty fiber statement is now explicit. However, the theorem is now wildly overgeneralized: for an arbitrary continuous map Φ : PrivateStrategy → Ω → ℝ from an arbitrary compact space, Set.range Φ need not be convex. Convexity is project geometry from private randomization, not a consequence of compactness and continuity. This stub must either be tied to the actual ProfileRealizationSetup/private-kernel structure or include an explicit affine/convex-realization hypothesis.

krn_borel_right_inverse: addressed.
The statement now has the right measurable-selection flavor: standard Borel structures, continuous surjection, compact nonempty fibers, and a measurable right inverse. hfib_ne is redundant with surjectivity but harmless. Minor modeling note: downstream use probably wants the codomain to be the subtype W rather than an arbitrary Y, but this is not a blocker.

kernel_infimum_epsilon_selection: partially addressed, still mismatched.
Replacing the naked ⨅ equality by ε-selection plus the lower bound was the right move. The problem is the hypothesis bundle. The use site adversary-infimum-pointwise is for bounded measurable profile maps w : M → W, so g s m = s · w m is generally measurable, not continuous in m. The patched stub requires ∀ s, Continuous fun m => g s m, which is too strong for the decomposition’s own invocation. It also lacks the standard-Borel/Polish or explicit measurable-selection hypotheses needed to produce measurable ε-minimizers. This one will not instantiate at the intended adversary-infimum use site.

jankov_von_neumann_universal_selection: mostly addressed.
The placeholder is gone, and the analytic graph plus universally measurable selector shape is faithful to the intended descriptive-set-theory theorem. Minor Lean-shape caveat: depending on Mathlib’s AnalyticSet API, the final formal statement may need explicit BorelSpace or compatible topology/measurable-space assumptions. This is a small typeclass polish issue, not a semantic blocker.

geps_borel_selector_upgrade: partially addressed.
The new GepsRegularity structure is much better than True. But the selection theorem is still too broad as stated: closed-valued Borel graph selection generally needs a Polish/compact or σ-compact-section target condition. The theorem has SecondCountableTopology and StandardBorelSpace, but no CompactSpace M, PolishSpace M, or equivalent section σ-compactness assumption. Since the intended M is a compact support subspace, adding compactness or Polishness is faithful and would make the selector shape much safer.

bayes_posterior_as_conditional_barycenter: not sufficiently addressed.
This is the biggest “painted door” in the patch. The theorem’s load-bearing conclusion is essentially assumed as hP_post : ∀ ω, ∀ᵐ m ∂q, P m ω = ∫ s, coord s ω ∂ρ m; the conclusion merely bundles the finitely many coordinatewise a.e. identities into one ∀ᵐ m, ∀ ω. That may be a useful finite-intersection lemma, but it is not the Bayesian posterior-as-conditional-barycenter theorem advertised by the external dependency. The real stub needs hypotheses tying q, ρ, and P to the joint law/disintegration and posterior-law consistency, rather than assuming the coordinate barycenter identity directly.

support_function_measurable_integrated_separation: partially addressed, still mismatched.
The RHS is now meaningful convex-analysis syntax, but it is not the integrated Hall theorem described in the dependency. The source/decomposition wants measurable-event support-function Hall inequalities; the proposed statement gives fixed-functional q-a.e. domination:
∀ ℓ, ∀ᵐ m, ℓ (P m) ≤ sSup (ℓ '' B m).
In an infinite-dimensional or nonseparable dual setting, ∀ ℓ, ∀ᵐ m is weaker than the pointwise a.e. statement ∀ᵐ m, ∀ ℓ, because the null set may depend on ℓ. Either restrict E to the finite-dimensional belief space actually used, provide a countable separating family, move the quantifier inside the a.e. statement, or state the real integrated-event Hall form.

New Concerns Introduced

Generic false theorem shapes.
profile_geometry_import and geps_borel_selector_upgrade are stated at a level of generality that the source proof does not justify. This is scope drift, not harmless abstraction.

Over-specification blocking instantiation.
kernel_infimum_epsilon_selection now requires continuity in the message variable, but the menu-engine use sites only provide Borel/measurable labelings. This would block adversary-infimum-pointwise.

Conclusion hidden as a hypothesis.
bayes_posterior_as_conditional_barycenter assumes the coordinate barycenter identity and then concludes its finite bundled form. That is not the advertised external theorem.

Integrated Hall drift.
support_function_measurable_integrated_separation has become a pointwise support-function equivalence over fixed linear functionals, not the integrated measurable-event Hall equivalence described elsewhere in the audit.

Proposal/document mismatch on Fubini.
dep_audit.md correctly promotes Measure.integral_compProd; dep_audit_proposed.md still presents the old ordinary product Fubini candidate list. Tiny paper-cut, but the formalizer reads paper-cuts as trapdoors.

Coverage Audit

The patch appears intended to touch exactly the 8 flagged items: the Fubini row plus the 7 placeholder stubs. No substantive regression is visible in the other confirmed Mathlib matches.

However, dep_audit_proposed.md still contains stale classification/candidate text outside the patched stubs, especially around the Fubini candidate ordering. Before formalization, synchronize the proposal file with the verified audit table so there is a single source of truth.
