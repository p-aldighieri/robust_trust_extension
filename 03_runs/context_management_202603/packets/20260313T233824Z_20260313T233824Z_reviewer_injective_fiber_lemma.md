...
Injective-fiber lemma review

## Goal

Review only the new injective-fiber lemma claiming that q*-a.e. injectivity of m -> bar w*(m) is enough, on the already-banked reduced-side inputs, to recover the Needed posterior version for the chosen raw lift. Check whether it is mathematically correct as a strengthened sufficient condition, and what exact caveat is needed about how it should be presented.

## Hard Constraints

- Do not let this strengthened condition be reframed as the preferred general theorem unless the mathematics really supports that.
- Do not reopen unconditional routes.
...
Return a reviewer verdict focused only on this injective-fiber lemma: PASS if correct as written or with only local wording fixes; otherwise identify the first precise defect and the smallest viable repair.

## Proof-State Update Target

If PASS, bank the injective-fiber lemma as a valid strengthened sufficient condition, while keeping the main least-strengthened route anchored at the explicit Needed assumption.
...
### FILE: /Users/p-aldighieri/Library/CloudStorage/OneDrive-Personal/Economia/RA Piotr/robust_trust_extension/Context Management/logs/20260313T230943Z_prover_injective_fiber_probe_response.md
