...
Continuity-conditioned exact version-and-patching saddle lemma

## Goal

Review only the local lemma claiming that, once the continuity-based exact raw lifting lemma is available upstream, the q*-null-set monotone patch of the deterministic reduced saddle preserves adviser-side saddle inequalities against every admissible raw adviser kernel. Check the agent-side equality step, the adviser-side inequality step, and whether any hidden assumption is used beyond the already-stated continuity-conditioned setup.
...
Return a reviewer verdict focused only on this patching lemma: PASS if correct as written or with only local cosmetic fixes; otherwise identify the first precise mathematical defect and the smallest repair if one exists.

## Proof-State Update Target

If PASS, bank the continuity-conditioned exact version-and-patching saddle lemma as trustworthy and treat the least-strengthened exact route as locally reopened through the patching hinge.
