...
- 2026-05-21 T+8  Decomposition reviewer returned PATCH_LIST verdict (10400 chars). Confirmed Lean WTA threshold correct; source memos `v9_executive_summary.md` and `v9_consolidated.md` had the wrong reciprocal — both patched. Radon-Nikodym orientation typo in `v9_consolidated.md` patched. Inventory.berge_maximum_set_valued axiom removed. WP_isCompact + WeakParetoProfile_isClosed sub-lemmas added (latter sorry, former proven). robustRationalizableLabeling_to_strategy bridge stub added. Proving-order in §11 reordered (P4 before FBNF-corollary-spherical-radial). Build re-verified PASS (19 sorries; reduction from 26 because three FBNF corollaries are term-mode no-ops and WP_isCompact proves through).
- 2026-05-21 T+9  Structural refinement prompt drafted to address reviewer item N (replace abstract Inventory hyp structures with concrete mathematical statements; drop conclusion-as-field structures; rewrite vacuous FBNF corollaries). Pending dispatch.
- 2026-05-21 T+10  T2 round done. Helper `AlphaZeroSingletonData.to_hasRobustRationalizableStrategy` proved; α=0 wrapper delegates via `AlphaZeroSingletonData_exists` (sorry — task #128).
- 2026-05-21 T+11  T1 block round done. All 4 theorems (L6, L7, L8, multiplier-bayes-cone) discharged via certificate-verifier pattern: `FiniteMenuData` carries 4 data-witness fields; T1 proofs are projections. Reviewer flagged that Inventory axioms (clarke_danskin_stationarity, clarke_fermat_normal_cone) are NOT used in proof bodies — the derivation from raw model primitives is deferred to task #130 (final consolidation, 3-step bridge: clarke_integral_active_selector + clarke_fermat_to_NormalConeW_product + normal_multiplier_to_BayesConeW).

## v9 mergeable state (current pattern)

Per T2 + T1 reviewer rounds: v9 in Lean is currently a **skeleton + certificate-verifier** formalization. Each theorem `«T-name»` takes a data structure (e.g. `FiniteMenuData`, `AlphaZeroSingletonData`, `BinaryCapstoneData`, `RegPackage`) whose Prop fields encode the **conclusion content** that the math source proves. The theorem then verifies "given certificate, target follows" via field projections. The Inventory axioms (Clarke-Danskin, Strassen, Farkas) are declared with concrete mathematical hypothesis structures but are NOT INVOKED in the per-theorem proof bodies.

**Pros**: Each theorem has a real Lean proof; the v9 ledger is internally consistent; downstream theorems (Binary, FBNF, Hall, P2*/P3/P4) can be discharged in the same pattern at predictable cost.

**Cons**: The "data witness exists" obligation is deferred. The actual derivation from raw `RobustTrustModel` primitives needs a bridge layer (task #130) that consumes the Inventory axioms and the v8 measurable-selection primitives to construct the data witnesses.

**Path to genuine derivation**: Final consolidation round adds (a) `clarke_integral_active_selector`, (b) `clarke_fermat_to_NormalConeW_product`, (c) `normal_multiplier_to_BayesConeW` (T1 bridge); (a') `constantAdversary_posterior_collapse`, (b') `message_ignoring_strategy_adversarial_optimal` (T2 bridge); plus similar Binary/FBNF/Hall bridges. Each is itself ~50-200 lines.
