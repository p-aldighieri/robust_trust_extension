...
========
ROLE: user (id=79998099-a9a1-4546-9fc3-2497c5174e1f)
========
...
- **AXLE check status**: This main.lean was checked via python -m mathpipeprover.cli axle check at lean-4.29.0 with --timeout 600. Result: okay=true, zero compile errors (all sorry warnings as expected). So the signatures TYPECHECK; your job is to verify they MEAN the right things.
- **Pass-1 history**: The formalizer's first attempt had 405 compile errors (root cause: Type* polymorphism in RobustTrustModel). Pass-2 (this file) replaced Type* with Type and now compiles. Look for any side-effects of that change that could have weakened signatures.
- **Counts**: 70 theorem declarations (59 structurer lemmas + 6 sub-theorems + 1 main package + 9 inlined INVENTORY stubs equals 75, but some structurer-named items became structure or def, leaving 70 as theorems). 70 sorry bodies. 100 structure/def declarations. 1440 lines.
...
- PATCH_SMALL: 1-2 signature tweaks; formalizer fixes in one pass.
- PATCH_BIG: Multiple signatures need rework; systematic translation issues.
- REDO: Main theorem signature does not match the English claim, many lemmas semantically wrong, OR any axiom / native_decide / unsafe was introduced.

## Specific items to audit (from the structurer + orchestrator chain of trust)

1. **Atomlessness scope.** Atomlessness of τ MUST be a hypothesis ONLY of wta_no_free_dust (and structures that bundle it like AtomlessTauSharpness). Tier 1a/1b/2 theorems must NOT have an atomlessness hypothesis. Look for this regression.
2. **Tier 2 hypothesis bundle.** tier2_qae_robust_rationalizability_under_menu_Hall must take BOTH ExactContact AND MenuHall as hypotheses (NOT just MenuHall).
3. **Reverse strategy lift.** Tier 1a theorem must conclude existence of σ* ∈ AgentStrategyFull (full Σ on Δ Ω × Θ), not just a restricted M-strategy. The bridging lemmas restricted-agent-strategy-extends-to-full and full-restricted-Ustar-equivalence should be present and used.
4. **Tier 2 posterior identity.** posterior-disintegration-menuHall-kernel-coincides lemma must exist and be placed between per-message-Bayes-optimality and tier2_qae_robust_rationalizability_under_menu_Hall in the DAG.
5. **κ identity for Tier 2.** The Tier 2 chosen adversary β* must be the menu-Hall kernel κ (menuHall-adversary-kernel-identity), NOT the deterministic exact-contact selector.
6. **Payoff layer.** Aligned, Misaligned, Mixture, Robust payoffs should be distinct definitions (not bundled).
7. **WTA cone intersection** (Lemma 7 — high vacuous-lemma risk per Pedro's note): verify wta_cone_intersection has the right hypothesis profile (support λ = I, ∀ i ∈ I, 0 < λ i, ∑ λ = 1) and doesn't accidentally become vacuously true.
8. **Dust label typing.** null-dust-data should be over the subtype N (or qN-a.e. predicates with explicit m ∈ N hypothesis). Note that the formalizer was instructed to rename Greek λ to lam/lamN.
9. **9 INVENTORY.lean stubs inlined.** Inlined under namespace Inventory at top of file. Check signatures match the source INVENTORY.lean (attached) and the v3 stub statements in dep_audit_proposed.md.

## Output Contract

Return the deliverable inline in this chat. Plain markdown only.
...
- axiom declarations introduced: list
- native_decide / unsafe / custom Decidable: list
- sorry outside theorem bodies (in def): list
- MATHEMATICAL_CONCERN blocks: list
...
Translation, not mathematics. Don't add hypotheses the source didn't state. Never use axiom. Don't use native_decide.


## Context Packet


## main.lean (AXLE-compile-clean; reviewer audit target)
...
## Source INVENTORY.lean (v3 stubs)
