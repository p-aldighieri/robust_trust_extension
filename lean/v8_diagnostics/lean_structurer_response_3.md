...
========
ROLE: user (id=957f6066-2d4d-4aab-a4d2-19c5b4d93936)
========
You are the Lean Structurer in the Lean post-processing module of the soft-scaffolding workflow.

This is PASS 3 — a narrower PATCH_BIG rework. Pass-2 reviewer verdict: PATCH_BIG with implicit_assumptions_absorbed: 3, object_definition_concerns: 5. The reviewer explicitly confirmed that 90% of the pass-1 issues are now addressed faithfully (payoff layer split, κ identity, Bayes-plausibility, ProfileRealizationSetup, ΔΩ→M bridge objects, JvN scope, main theorem split, Definition2QAEPredicate, menu-value split, profile-realization split, no-free-dust split, WTA cone-identification, witness classification, most external reclassifications). Four load-bearing seams remain. Your job is to fix EACH of them, and to NOT regress on anything that's already passing.

Re-emit the FULL revised decomposition (not a diff) in the same lean_structure format.
...
- Identify every mathematical *object* the proof introduces or relies on that needs Lean-side definition before any theorem can typecheck.
- Capture each item with a precise English statement and its mathematical *type signature* in plain words.
- Record dependencies between items.
- Mark each external result as MATHLIB_CANDIDATE or NON_MATHLIB.
- Do not formalize anything in Lean yet. Do not propose proof tactics.
- Do not add assumptions that are not in the English proof. If something is implicit, surface it as an IMPLICIT_ASSUMPTION item.

## Specific Patch Requirements (pass 3 — address EACH)

1. **Reverse strategy lift: restricted M-strategy → full Σ strategy.** The Tier 1a theorem currently produces only a restricted strategy. v8 states existence of σ* ∈ Σ (on full Δ Ω × Θ). Add:
   - object/lemma restricted-agent-strategy-extends-to-full: ∀ σM : AgentStrategyM, ∃ σFull : AgentStrategyFull, restrict σFull = σM.
   - lemma full-restricted-Ustar-equivalence: the restricted U_star and full U_star are the same value.
   - Update tier1a-value-optimality-and-epsilon-adversary to quantify over AgentStrategyFull (or explicitly package the full extension as part of the conclusion). The menu engine can still work internally over M, but the theorem statement must expose σ* on the full message space.

2. **Tier 2 posterior identity.** Add posterior-disintegration-menuHall-kernel-coincides:
   MenuHall κ → ∀ᵐ m ∂qκ, Pβ κ m = Pγα m
   (or define Pβ κ via the same coupling γα so the identity is definitional). Insert this lemma between per-message-Bayes-optimality and tier2-qae-robust-rationalizability-under-menu-Hall so that Definition2QAEPredicate κ σ* connects to the menu-Hall posterior.

3. **Atomlessness scope — remove from WTA environment.** SPLIT WTA-ternary-environment into:
   - WTA-ternary-algebra (Fin 3, vertices, μ0, cones, finite-coordinate algebra — used by wta-cone-intersection)
   - separate AtomlessTauSharpness hypothesis (used ONLY by wta-no-free-dust)
   Verify that wta-cone-intersection no longer references atomlessness in its dependencies or environment.

4. **Dust-label typing — subtype N or q_N-a.e. discipline.** The dust set N := {m ∈ M : positive q-mass and aligned best ∉ K_label⁻}. Currently wN, λ, I are defined only on N, but rowwise-support, Bayes-cone-calibration, adversarial-flow-disintegration-data talk as if they are available for all m ∈ M. Choose ONE clean Lean shape and apply consistently:
   - Preferred: define q_N := q|_N as a measure on the subtype N : Set M (or Subtype (· ∈ N)); define kernels ρ : N → ProbabilityMeasure (Belief Ω); restate predicates over m : N (not m : M).
   - Alternative: keep predicates over m : M but add explicit m ∈ N → ... hypotheses (q_N-a.e. statements with explicit subtype proof).
   Apply to: rowwise-support, Bayes-cone-calibration, adversarial-flow-disintegration-data, dust-conditional-sources-satisfy-cones. Also add qN-supported-on-N or a subtype version of dust-disintegration.

5. **null-dust-data: add load-bearing fields.** Add to null-dust-data:
   - λ_measurable: measurability of λ
   - λ_nonneg: ∀ m i, 0 ≤ (λ m) i
   - λ_sum_one: ∀ m, ∑ i, (λ m) i = 1
   - λ_support_nonempty: ∀ m, (support (λ m)).Nonempty
   - **wN_eq_mixed_label**: ∀ m : N, wN m = wλ (λ m)   (this is the load-bearing identity linking I(m) to the actual dust label)

6. **α-case split surfaced in DAG.** Add lemma positive-dust-mass-impossible-when-alpha-one:
   α = 1 → τ N = 0 → ¬ PositiveQMass N κ.
   Then wta-no-free-dust splits on (α = 1 ∨ α < 1). The α = 1 trivial branch must be a Lean lemma, not just prose.

7. **Geps-selector-exists — pick total or τ-a.e.** Replace the ambiguous "for every s or τ-a.e. s as required" with a single explicit measurability target. Preferred: a total admissible selector (matching the source's jankov-von-neumann-universal-selection upgrade). If that's not feasible, state τ-a.e. selector plus an arbitrary default outside the full-measure set; the type signature must reflect the chosen shape.

8. **sigma-star-realization-and-optimality split.** Split into three lemmas:
   - wstar-profile-map-implemented: profile map for w* is Borel and implementable by an agent strategy.
   - wstar-payoff-equals-F-Cdagger: aligned payoff at w* equals max over C†; misaligned infimum equals min over C†.
   - sigma-star-robust-optimal: U(σ*) = U_star.
   This makes the payoff identity visible instead of hiding it inside realization.

9. **dust-conditional-sources-satisfy-cones split.** First prove dust-disintegration-over-subtype-N; then prove cone support and barycenter calibration over m : N. Avoid repeated a.e. coercion.

10. **standard-borel-disintegration reclassification.** Demote from NON_MATHLIB → MATHLIB_CANDIDATE (or at least mark as "Mathlib_or_partial_import"). Mathlib 4 (at lean-4.29.0) appears to have Mathlib.Probability.Kernel.Disintegration.StandardBorel. Final classification is the dep-auditor's call, but flag the change.

11. **Belief definition correction.** Belief Ω := {s : Ω → ℝ // (∀ ω, 0 ≤ s ω) ∧ ∑ ω, s ω = 1} (coordinatewise nonnegativity, not a scalar 0 ≤ s).

12. **Main theorem package — restore full-Σ exposure.** The main theorem package (robust-trust-infinite-extension-v8-package) currently over-corrects toward "restricted strategy" language. The published statement should quantify over σ : AgentStrategyFull. The menu engine can be a private/auxiliary construction.

## Critical Discipline

- **Do not regress.** The reviewer explicitly confirmed pass-2's payoff split, κ identity, Bayes-plausibility, ProfileRealizationSetup, JvN scope, etc. are faithful. Keep them.
- **Do not introduce new scope drift.** Every new object/lemma must be traceable to the source v8 proof or to a reviewer-required bridge.
- **Decrement, don't increment, the implicit_assumptions_absorbed count.** Pass 1 had 4, pass 2 has 3. Pass 3 should have 0 or 1. Surface every assumption you bake in as an IMPLICIT_ASSUMPTION item explicitly.
...
... (full section, same format as pass 2)

## Main Theorem
... (statement should expose σ ∈ AgentStrategyFull, not just restricted)

## Lemmas
... (in dependency order, with the new bridging lemmas inserted)

## External Results Invoked
... (with `standard-borel-disintegration` demoted)

## Implicit Assumptions Surfaced
... (surface ALL implicit assumptions — aim for implicit_assumptions_absorbed = 0)

## Decomposition Notes
... (note the new bridging objects and the split of WTA-ternary-environment)
...
- Do not "improve" the proof, generalize a result, or fix a mathematical issue you spot mid-translation. If you notice a real mathematical concern, surface it as a MATHEMATICAL_CONCERN block at the end of your output and continue translating faithfully.
- Do not invent results, lemmas, definitions, or objects that are not in the upstream decomposition. Every Lean declaration you introduce must trace back to an item the structurer named (or, if you are the structurer, to the source proof).
- **Never use axiom.** Stubbed dependencies use theorem ... := sorry.
- Do not use native_decide or any tactic that delegates correctness to Lean's compiler rather than its kernel.
...
## Reviewer Feedback (verbatim — Lean Structurer Reviewer, pass 2)

review_control
verdict: PATCH_BIG
ready_for_dep_audit: false
recommended_next_phase: LEAN_STRUCTURER_PATCH
implicit_assumptions_absorbed: 3
object_definition_concerns: 5
Verdict

VERDICT: PATCH_BIG

Reason: The pass-1 patch mostly landed: the payoff layer is now split, κ is correctly identified as the Tier 2 adversary, posterior-law consistency and profile-realization are explicit, menu-Hall is scoped as a Tier 2 hypothesis, the q-a.e. Definition 2 reading is represented, and the sharpness package is substantially better decomposed. However, the revised DAG is still not ready for dependency audit. Four load-bearing seams remain under-specified: the restricted M strategy result is not lifted back to the paper’s full Σ; the Tier 2 posterior used by Definition2QAEPredicate is not identified with the Pγα posterior supplied by menu-Hall; the dust-label data are ill-typed unless the label/mixed-profile encoding is made explicit; and atomlessness is still overbundled into the WTA environment in a way that can silently strengthen wta-cone-intersection.

Opinion and Next Move

Send back to the structurer for one focused but real patch pass. Do not send to dependency audit yet. The decomposition is close enough that a redo is unnecessary, but the remaining object-shape issues would poison downstream Lean declarations if left as “obvious glue.”

Detailed Review
Object-Definition Audit

Most pass-1 object concerns were addressed faithfully. In particular, aligned-payoff, misaligned-payoff, mixture-payoff, robust-payoff, U-star, and is-adversarial are now distinct, which fixes the old payoff-layer ambiguity. menuHall-adversary-kernel and menuHall-adversary-kernel-identity correctly prevent the deterministic exact-contact selector from being smuggled into Tier 2.

message-restriction-bridge is still only half-closed. The source theorem’s strategy lives in full Σ, i.e. messages in Δ Ω, while the menu engine works on M. The revised DAG has strategy-restriction-to-M, outside-M-messages-irrelevant, and adversary-kernels-restrict-to-M, but it lacks the reverse construction: a Borel restricted strategy on M must be extended to a full strategy on Δ Ω by arbitrary/default completion outside M. This matters because tier1a-value-optimality-and-epsilon-adversary currently states existence of a restricted strategy, while v8 states existence of σ* ∈ Σ.

posterior-disintegration has two notationally distinct posteriors, Pβ and Pγα, but the DAG never proves that for the menu-Hall kernel κ, the Pβ used inside Definition2QAEPredicate κ σ* is q-a.e. equal to the Pγα supplied by menu-Hall-assumption. menuHall-adversary-kernel-identity proves equality of message marginals, not equality of conditional posteriors. That is not enough in Lean.

WTA-ternary-environment should not itself carry atomlessness if wta-cone-intersection depends on it. The cone-intersection theorem is purely finite-coordinate probability algebra and should not assume atomlessness. Either split this into WTA-ternary-algebra plus AtomlessTauSharpness, or make atomlessness a separate hypothesis only of wta-no-free-dust.

null-dust-data is missing a load-bearing field: wN_eq_mixed_label : ∀ m : N, wN m = wλ (λ m). The source’s no-free-dust theorem uses I(m) := supp λ(m) specifically because λ(m) encodes the dust label. Without this equality, the cones can float free of the actual label.

rowwise-support and Bayes-cone-calibration are currently ill-shaped around the dust subset. Since wN, λ, and I are defined on N, predicates over all m : M must either use m ∈ N → ..., extend the label data outside N, or define q_N and ρ_m over the subtype N. The cleanest Lean shape is to disintegrate over the subtype measure on N.

standard-borel-disintegration is classified as NON_MATHLIB, but current Mathlib documentation includes disintegration modules including Mathlib.Probability.Kernel.Disintegration.StandardBorel; this should be checked at the pinned toolchain before stubbing it as a project external. 
Lean Prover Community

Minor modeling note: Belief := {s : Ω → ℝ // 0 ≤ s ∧ ∑ω s ω = 1} should be rendered as coordinatewise nonnegativity, e.g. ∀ ω, 0 ≤ s ω, not a scalar 0 ≤ s.

Faithfulness Audit

Item: tier1a-value-optimality-and-epsilon-adversary, sigma-star-realization-and-optimality, restricted-agent-strategy-space-on-M
Issue: claim drift / wrong final object shape.
Why it matters: v8’s Tier 1a theorem produces σ* ∈ Σ, not merely a restricted M-indexed section. The source allows the menu engine to work over M, but the final theorem must return a full agent strategy on Δ Ω × Θ.
Suggested repair: Add restricted-agent-strategy-extends-to-full:
∀ σM : AgentStrategyM, ∃ σFull : AgentStrategyFull, restrict σFull = σM.
Then add restricted-optimality-lifts-to-full-Ustar or fold it into Tier 1a. Final theorem statements should quantify over AgentStrategyFull or explicitly package the full extension.

Item: definition2-qae-predicate, menu-Hall-posterior-calibration-unpack, tier2-qae-robust-rationalizability-under-menu-Hall
Issue: missing posterior identity.
Why it matters: menu-Hall gives Bayes-optimality under Pγα; Definition 2 is stated using Pβ*. Since Tier 2 sets βstar := κ, Lean needs an explicit q-a.e. identification Pκ = Pγα. Equality of qκ and (γα)_2 alone does not identify chosen conditional versions.
Suggested repair: Add posterior-disintegration-menuHall-kernel-coincides:
MenuHall κ → ∀ᵐ m ∂qκ, Pβ κ m = Pγα m, or define Pβ κ by the same coupling γα so the identity is definitional.

Item: WTA-ternary-environment, wta-cone-intersection
Issue: added assumption / overbundled object.
Why it matters: wta-cone-intersection should not import atomlessness through its environment object. The source cone lemma uses only support on K_I^-, barycenter in B_I, and finite coordinate algebra.
Suggested repair: Split the WTA object. Use WTA-ternary-algebra for Fin 3, vertices, μ0, and cones. Use a separate Atomless τ hypothesis only in wta-no-free-dust.

Item: null-dust-data
Issue: missing load-bearing field.
Why it matters: No-free-dust applies cone intersection to the support I(m) of the mixed label actually assigned to dust message m. Without wN m = wλ (λ m), I(m) could be arbitrary bookkeeping unrelated to the label.
Suggested repair: Add fields for λ_measurable, λ_nonneg, λ_sum_one, support_nonempty, and explicitly wN_eq_mixed_label.

Item: rowwise-support, Bayes-cone-calibration, adversarial-flow-disintegration-data
Issue: partial function / subtype mismatch.
Why it matters: wN, λ, and I are defined only on the dust set N, while the current predicates talk as if they are available for all messages in M. Lean will not let this pass without an unsafe default or a hidden extension.
Suggested repair: Make q_N a measure on the subtype N, with kernels ρ : N → ProbabilityMeasure (Belief Ω), or rewrite predicates as q_N-a.e. statements with an explicit proof that m ∈ N.

Item: wta-no-free-dust, dust-positive-mass-forces-mu0-atom
Issue: α-case split is surfaced in prose but not represented in the lemma DAG.
Why it matters: dust-positive-mass-forces-mu0-atom requires α < 1; the final theorem has no such hypothesis because the α = 1 case is trivial. Lean still needs that trivial branch.
Suggested repair: Add positive-dust-mass-impossible-when-alpha-one:
α = 1 → τ N = 0 → ¬ PositiveQMass N κ.
Then wta-no-free-dust splits on α = 1 ∨ α < 1.

Item: Geps-selector-exists
Issue: ambiguous quantifier strength.
Why it matters: The statement says “for every s or τ-a.e. s as required,” while the type signature states ∀ s. The kernel construction only needs τ-a.e. support, but a deterministic kernel from a Borel function is simplest with a total selector.
Suggested repair: Pick one. Preferred: prove a total admissible selector if the external supports it; otherwise state an a.e. selector plus define arbitrary default values off the full-measure set.

Completeness Audit

Missing object / lemma / external result: restricted-agent-strategy-extends-to-full.
Where in the English proof it appears: The profile-realization sub-lemma produces an agent strategy from a Borel w : M → W, but Tier 1a’s theorem is stated as σ* ∈ Σ; the full-message completion is the bridge between those.

Missing object / lemma / external result: full-restricted-Ustar-equivalence.
Where in the English proof it appears: The menu engine computes the original robust value, not a new restricted-game value. The current bridge has restriction and outside-irrelevance lemmas, but no final equality showing the restricted U_star and full U_star are the same object.

Missing object / lemma / external result: posterior-disintegration-menuHall-kernel-coincides.
Where in the English proof it appears: Tier 2 says the chosen β* is the menu-Hall kernel and the posterior in Definition 2 is induced by the mixture law for that same kernel.

Missing object / lemma / external result: selected-labeling-payoff-equals-pruned-menu-F.
Where in the English proof it appears: After closure pruning, the realized labeling w* has aligned payoff equal to the max over C† and misaligned infimum equal to the min over C†. sigma-star-realization-and-optimality currently jumps over this equality.

Missing object / lemma / external result: qN-supported-on-N or subtype version of dust-disintegration.
Where in the English proof it appears: No-free-dust disintegrates ν_N over messages in N and then applies cone conditions to q_N-a.e. dust message.

Missing object / lemma / external result: dust-label-equals-mixed-profile.
Where in the English proof it appears: The theorem says I(m) encodes the dust label w_N(m)=w_{λ(m)}.

Decomposability Audit

Item: sigma-star-realization-and-optimality
Concern: too bundled.
Suggested split or merge: Split into:
...
Item: tier2-qae-robust-rationalizability-under-menu-Hall
Concern: it combines kernel identity, adversariality, posterior identity, q-a.e. Bayes optimality, and τ-a.e. domination.
Suggested split or merge: Keep the capstone, but insert the missing posterior-disintegration-menuHall-kernel-coincides between per-message-Bayes-optimality and Definition2QAEPredicate.

Item: wta-no-free-dust
Concern: α-case split currently lives in notes, not the DAG.
Suggested split or merge: Add the α=1 trivial lemma and make dust-positive-mass-forces-mu0-atom the α<1 branch.

Item: dust-conditional-sources-satisfy-cones
Concern: depends on a dust label map whose domain is not aligned with the measure domain.
Suggested split or merge: First prove dust-disintegration-over-subtype-N; then prove cone support and barycenter calibration over m : N. This avoids repeated a.e. coercion goblins.

Item: support-function-integrated-Hall-equivalence
Concern: not a blocker, but it should stay quarantined.
Suggested split or merge: Keep it auxiliary and out of the main Tier 2 DAG. Its hypotheses are stronger than the posterior-membership menu-Hall form.

Scope / Assumptions

Implicit assumptions silently absorbed from the English proof rather than surfaced:

A restricted M-indexed strategy can be extended to a full Δ Ω strategy without changing the objective.

For β = κ, the posterior object Pβ used by Definition 2 is q-a.e. the same conditional posterior as Pγα.

In the WTA sharpness layer, the dust message marginal q_N is either a measure on the subtype N or carries an explicit support proof allowing I(m) and wN(m) to be used q_N-a.e.

External results misclassified as MATHLIB_CANDIDATE when actually specialist:

The current NON_MATHLIB choices are mostly conservative and appropriate: profile-geometry-import, krn-borel-right-inverse, kernel-infimum-epsilon-selection, jankov-von-neumann-universal-selection, geps-borel-selector-upgrade, bayes-posterior-as-conditional-barycenter, and support-function-measurable-integrated-separation should remain specialist/project externals.

standard-borel-disintegration should be rechecked: Mathlib appears to have standard-Borel disintegration infrastructure now, so this may be a MATHLIB_CANDIDATE or partial-Mathlib import rather than a NON_MATHLIB stub.

hyperspace-blaschke-compactness can remain conservatively non-Mathlib until the compact-set Hausdorff API is confirmed.

measurable-maximum-and-argmax-selection remains correctly NON_MATHLIB; the measurable-selection content is too specialized to assume from baseline Mathlib.

Over-correction:

WTA-ternary-environment over-corrects by bundling atomlessness into an environment used by atomlessness-free lemmas.

The main theorem over-corrects toward “restricted strategy” language. The menu engine may be restricted internally, but the theorem package should expose the paper’s full Σ object.

Geps-selector-exists over-corrects by leaving an either/or measurability target; the formal statement must choose a precise admissibility level.

Pass-1 patch checklist:

Addressed faithfully: payoff layer split; Tier 2 κ identity; Bayes-plausibility consistency; ProfileRealizationSetup; ΔΩ→M bridge objects; JvN universal-measurability correction; main theorem split into six components; explicit predicates; nonemptiness assumptions; Tier 2 ExactContact ∧ MenuHall; Definition2QAEPredicate; menu-value equivalence split; profile-realization split; no-free-dust split; WTA cone-identification hypotheses; witness classification as behavioral artefact; most external reclassifications.

Not fully addressed: ΔΩ→M bridge lacks the reverse full-strategy lift; atomlessness is still baked into the WTA environment; no-free-dust dust-label typing is not Lean-ready; Tier 2 posterior identity is still missing.


## Current Decomposition (pass 2) — refine, do not redo
...
========
ROLE: assistant (id=bfea1f31-7ffb-4dc8-bde8-a02e81f64dba)
========
lean_structure
main_theorem: robust-trust-infinite-extension-v8-package
object_count: 53
