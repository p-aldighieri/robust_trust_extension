...
========
ROLE: user (id=dd623f4d-9b3d-45c6-8e6d-f53d29b87948)
========
...
- Flag confirmed candidates that depend on extra axioms in Mathlib (Classical.choice, propext, etc. used in non-standard ways).
...
## Audit Summary (sub-agent's output)

- Backend: Opus 4.7 general-purpose sub-agent (codex CLI not available on Windows host).
- Total externals audited: 17 (every external the structurer flagged).
- Buckets: confirmed=8, wrong_name_retry_exhausted=0, not_in_mathlib=9, axiom_dependent=0.
- Toolchain: lean-4.29.0 + Mathlib (the AXLE-pinned default).
- Note on probes: AXLE's lean-4.29.0 env requires import Mathlib (umbrella), not narrow imports. The sub-agent revised its probe template after a first attempt failed with "Imports mismatch."
- Note from sub-agent: MeasureTheory.Measure.disintegrate at 4.29.0 uses the typeclass-based [ρ.IsCondKernel ρCond] signature rather than the older explicit-argument form. Downstream proof code may need to register the kernel as an IsCondKernel instance.

## Specific Items to Verify

The 9 not_in_mathlib items:
1. measurable-maximum-and-argmax-selection (Aliprantis–Border type)
2. profile-geometry-import (project-specific kernel-space geometry)
3. krn-borel-right-inverse (Kuratowski–Ryll-Nardzewski selection)
4. kernel-infimum-epsilon-selection (packaged kernel-optimization identity)
5. hausdorff-support-function-lipschitz (support-function Lipschitz on hyperspace)
6. jankov-von-neumann-universal-selection (descriptive set theory)
7. geps-borel-selector-upgrade (regularity upgrade beyond JvN)
8. bayes-posterior-as-conditional-barycenter (project Bayesian glue)
9. support-function-measurable-integrated-separation (measurable/integrated separation)

For each: spot-check whether Mathlib has an equivalent at lean-4.29.0 that the sub-agent missed. If you confirm any of them is genuinely not in Mathlib, leave the bucket as-is and confirm the INVENTORY.lean stub plan is faithful. If you suspect one IS in Mathlib, name the candidate and suggest a re-probe.

The 8 confirmed items — verify semantics:
1. finite-dimensional-simplex-compactness → stdSimplex (with convex_stdSimplex, isCompact_stdSimplex)
2. fubini-tonelli-kernel-integrals → MeasureTheory.integral_prod
3. hyperspace-blaschke-compactness → TopologicalSpace.NonemptyCompacts.instCompactSpace
4. weierstrass-extreme-value → IsCompact.exists_isMaxOn
5. standard-borel-disintegration → MeasureTheory.Measure.condKernel
6. support-function-pointwise-separation → iInter_halfSpaces_eq
7. nonnegative-integral-zero-iff-zero-ae → MeasureTheory.integral_eq_zero_iff_of_nonneg_ae (verified)
8. atomless-singleton-null → MeasureTheory.NoAtoms.measure_singleton (verified)

For each: do the Mathlib hypotheses match what the structurer's lemmas actually need? Quantifier scope, ambient typeclass requirements, generality.
...
- For each confirmed Mathlib result that uses Classical.choice, propext non-trivially, large cardinals, or other extra-axiom machinery: list it with a note.
...
  If you disagree: suggest a candidate the sub-agent should re-probe.

### INVENTORY.lean Stub Plan Review

- Are all `not_in_mathlib` items captured in the INVENTORY.lean stub plan from `dep_audit_proposed.md`? (yes/no — list any gaps)
- Are stub signatures correct shape?
- Any stubs that should be proved inline in the main file instead of stubbed?
...
- axiom_dependent_candidates is documentation, not a veto.
...
## Verified Dep Audit (dep_audit.md) — what the sub-agent produced
...
| fubini-tonelli-kernel-integrals | Iterated integration against product measures and Markov kernels is valid; expected payoffs can be rearranged over states, types, actions, messages, and kernels. | MeasureTheory.integral_prod | Mathlib.MeasureTheory.Integral.Prod (via import Mathlib) | confirmed | 5 | 0 | Bochner Fubini confirmed. Kernel-side MeasureTheory.Measure.compProd (cand4) also confirmed at Mathlib.Probability.Kernel.Composition.MeasureCompProd. |
...
## Original Dep Audit Proposal (dep_audit_proposed.md) — candidates the sub-agent probed
...
========
ROLE: assistant (id=2388a060-81a2-48ec-9e84-1edea4d1b330)
========
review_control
verdict: PATCH_SMALL
ready_for_formalization: false
recommended_next_phase: DEP_AUDIT_PATCH
needs_inventory_lean_update: true
axiom_dependent_candidates: []
semantics_mismatch_count: 1
Verdict

VERDICT: PATCH_SMALL

Reason: The audit is mostly sound. The 9 not_in_mathlib classifications look essentially correct, and no broad re-audit is needed. However, one confirmed item is too coarse semantically for the proof DAG: fubini-tonelli-kernel-integrals → MeasureTheory.integral_prod confirms ordinary product-measure Bochner Fubini, but the use sites need Markov-kernel composition and kernel iterated-integral rearrangements. Mathlib has strong kernel compProd infrastructure, but the audit should re-probe the exact kernel-side Bochner statements or explicitly route through verified lintegral_compProd plus bounded-real/integrability conversion. 
Lean Community
+2
Lean Community
+2

Formalization should not begin yet. This is not a redo, just a small patch: re-check the kernel Fubini package and update the INVENTORY.lean stubs that currently contain placeholder hypotheses or conclusions such as True.

Opinion and Next Move

Run a targeted dependency patch, then proceed to the Lean formalizer. The patch should do two things: first, re-probe kernel integration names beyond MeasureTheory.integral_prod; second, replace the weakest proposed stubs with faithful theorem-shaped statements before the formalizer starts building around them.

Detailed Review
Confirmed Matches, Semantics Audit

external-slug: finite-dimensional-simplex-compactness
Confirmed Mathlib name: stdSimplex, with convex_stdSimplex, isCompact_stdSimplex
Concern: Accept. The semantics match compact convex finite probability simplex once the project’s Belief Ω is modeled as, or bridged to, the subtype of stdSimplex ℝ Ω. Mathlib also exposes the coordinate nonnegativity and sum-one projection lemmas, so the finite coordinate algebra has the right hooks. The “finite dot-products are continuous” part is still local finite-sum glue, not supplied by the simplex lemma itself. 
Lean Community

Suggested action: accept.

external-slug: fubini-tonelli-kernel-integrals
Confirmed Mathlib name: MeasureTheory.integral_prod
Concern: This is the one real semantics mismatch. integral_prod is ordinary product-measure Bochner Fubini. The English dependency and many use sites are about Markov kernels, τ.compProd β, iterated kernel integrals, a.e. conversion under kernel products, and deterministic kernels. Mathlib does have MeasureTheory.Measure.compProd, rectangle formulas, and lintegral_compProd, but the final confirmed name by itself is not enough for the proof as structured. 
Lean Community
+2
Lean Community
+2

Suggested action: re-probe kernel-side candidates, especially any AXLE-available Bochner theorem for Measure.compProd; otherwise document a local conversion path from lintegral_compProd or product-measure Fubini after expanding the kernel composition.

external-slug: hyperspace-blaschke-compactness
Confirmed Mathlib name: TopologicalSpace.NonemptyCompacts.instCompactSpace
Concern: Accept. This correctly captures compactness of nonempty compact subsets of a compact ambient space in the Vietoris topology. Mathlib also has the metric instance and dist_eq identifying the metric distance with Hausdorff distance, so the Hausdorff formulation is available once the ambient type is metric compact. 
Lean Community
+1

Suggested action: accept.

external-slug: weierstrass-extreme-value
Confirmed Mathlib name: IsCompact.exists_isMaxOn
Concern: Accept, with ordinary Lean bookkeeping. The theorem needs a compact set, a nonempty witness, and ContinuousOn f s; the minimum version has the analogous lower-order topology assumptions. This is a direct match for pointwise extrema, not for measurable selection. 
Lean Community

Suggested action: accept.

external-slug: standard-borel-disintegration
Confirmed Mathlib name: MeasureTheory.Measure.condKernel
Concern: Accept. The semantic match is good for finite measures on products with a standard-Borel conditional target. The audit correctly notes the important Lean detail: Measure.disintegrate is typeclass-based via [ρ.IsCondKernel ρCond], so downstream code must either use the canonical ρ.condKernel instance or register the conditional-kernel instance before rewriting. 
Lean Community
+2
Lean Community
+2

Suggested action: accept, but formalizer should normalize the product-coordinate convention early.

external-slug: support-function-pointwise-separation
Confirmed Mathlib name: iInter_halfSpaces_eq
Concern: Accept, but this is not a one-line support-function theorem. Mathlib’s statement is in terms of continuous linear functionals and an intersection of half-spaces with an existential bound over s. The project’s statement uses continuous affine functionals and support functions. That reduction is standard, but the formalizer must add the local glue: affine-to-linear plus constants, and converting ∃ y ∈ C, l p ≤ l y into the chosen support-function notation, usually using compactness and nonemptiness. 
Lean Community

Suggested action: accept, with a local wrapper lemma.

external-slug: nonnegative-integral-zero-iff-zero-ae
Confirmed Mathlib name: MeasureTheory.integral_eq_zero_iff_of_nonneg_ae
Concern: Accept. The hypotheses match once the use site supplies integrability and derives ∫ f = 0 from 0 ≤ᵐ[ae μ] f plus ∫ f ≤ 0. The English statement says nonpositive expectation, while the Mathlib lemma is an equality iff, so one small arithmetic wrapper is needed. 
Lean Community

Suggested action: accept.

external-slug: atomless-singleton-null
Confirmed Mathlib name: MeasureTheory.NoAtoms.measure_singleton
Concern: Accept. This is a direct semantic match.
Suggested action: accept.
...
No confirmed candidate appears to rely on extra axioms in a non-standard or suspicious way. Some results are noncomputable and use standard Mathlib classical infrastructure, especially disintegration and selection-flavored constructions, but that is documentation-worthy rather than a veto. The axiom_dependent_candidates list should stay empty.

Items Bucketed as "Not In Mathlib", Spot-Check

external-slug: measurable-maximum-and-argmax-selection
Sub-agent's reasoning: Mathlib has pointwise compact extrema, not a measurable maximum theorem with measurable argmax selectors.
Your assessment: agree. IsCompact.exists_isMaxOn is only pointwise and does not provide measurable dependence on the parameter.
If you disagree: n/a.

external-slug: profile-geometry-import
Sub-agent's reasoning: Project-specific compact standard-Borel topology and profile-map geometry for private kernels.
Your assessment: agree. This is not Mathlib material. Generic compact-image or convexity lemmas do not construct the private-kernel topology, prove continuity/surjectivity of the profile map, or identify compact fibers.
If you disagree: n/a.

external-slug: krn-borel-right-inverse
Sub-agent's reasoning: Mathlib has choice-theoretic right inverses, not KRN-style measurable right inverses.
Your assessment: agree. Function.surjInv is not a measurable selection theorem.
If you disagree: n/a.

external-slug: kernel-infimum-epsilon-selection
Sub-agent's reasoning: Mathlib has pointwise minima and deterministic-kernel tools, not the packaged measurable ε-selector plus kernel-infimum identity.
Your assessment: agree. This is specialist measurable optimization over kernels. The exact Lean statement should be patched before formalization, because an equality over ⨅ β : Kernel S M in ℝ is likely brittle unless the order/integrability setup is pinned down.
If you disagree: n/a.

external-slug: hausdorff-support-function-lipschitz
Sub-agent's reasoning: Mathlib has Hausdorff distance and distance-to-set Lipschitz lemmas, but no support-function Lipschitz theorem.
Your assessment: agree. Mathlib has the nonempty compact Hausdorff metric apparatus and Lipschitz lemmas for distance-to-set or set operations, but not the exact “support extrema of a continuous linear functional vary Lipschitzly with Hausdorff distance” package. 
Lean Community
+1

If you disagree: n/a.

external-slug: jankov-von-neumann-universal-selection
Sub-agent's reasoning: No Mathlib analytic-set or JvN selection theorem.
Your assessment: agree on the bucket, but correct the note. Current Mathlib documentation does expose MeasureTheory.AnalyticSet and related analytic-set infrastructure, including Lusin separation. What I do not see is the Jankov-von-Neumann universally measurable selector theorem needed here. 
Lean Community

If you disagree: n/a, but reword the inventory comment so it does not claim analytic sets are wholly absent.

external-slug: geps-borel-selector-upgrade
Sub-agent's reasoning: Specific ε-contact correspondence regularity upgrade beyond JvN; pointwise minima do not supply it.
Your assessment: agree. This is a project-specific selector theorem.
If you disagree: n/a.

external-slug: bayes-posterior-as-conditional-barycenter
Sub-agent's reasoning: Mathlib supplies disintegration and integration, not the model-specific posterior-process identity.
Your assessment: agree. This is Bayesian glue connecting the source-posterior law, finite coordinates, disintegration, and the project’s posterior definition.
If you disagree: n/a.

external-slug: support-function-measurable-integrated-separation
Sub-agent's reasoning: Pointwise separation is in Mathlib, but the measurable-correspondence and integrated Hall equivalence are not packaged.
Your assessment: agree. Mathlib’s pointwise Hahn-Banach separation does not supply the eventwise integrated support-function Hall equivalence.
If you disagree: n/a.

INVENTORY.lean Stub Plan Review

Are all not_in_mathlib items captured in the INVENTORY.lean stub plan from dep_audit_proposed.md?
Yes, all 9 are captured.

Are stub signatures correct shape?
Not yet. The inventory plan is faithful as an informal list, but not formalizer-ready. The following stubs need patching before Lean work begins:

profile_geometry_import: the statement quantifies an arbitrary W : Type* but concludes Convex ℝ (Set.univ : Set W). That requires vector-space structure on W, or better, represent W as a subtype or subset of Ω → ℝ. The theorem should also state nonempty fibers, not only compactness of fibers.

krn_borel_right_inverse: add standard-Borel or compatible topological/measurable hypotheses. Compactness of the source and continuity of Φ alone is not the measurable-selection theorem in the form the project uses.

kernel_infimum_epsilon_selection: avoid a naked exact ⨅ equality unless the target order and integrability are completely specified. An ε-selection inequality theorem may match the proof better and reduce lattice shrapnel.

jankov_von_neumann_universal_selection: the placeholder hG_analytic : True and conclusion-side True are too loose. Either make this a named black-box theorem with actual project predicates for analytic graph and universal measurability, or do not include it separately if geps_borel_selector_upgrade is the only theorem actually consumed.

geps_borel_selector_upgrade: replace hregular : True with the exact regularity predicate the proof assumes, or explicitly name a project-specific theorem that takes the graph and regularity bundle as a parameter.

bayes_posterior_as_conditional_barycenter: the current shape asserts the conclusion without the joint-law and disintegration hypotheses that make it true. Add the model law, conditional kernel, and coordinate barycenter assumptions.

support_function_measurable_integrated_separation: the right-hand side cannot be True. It must be the actual support-function Hall inequality predicate over measurable events and continuous affine tests. Otherwise the stub becomes a logic trapdoor with a decorative name.

Any stubs that should be proved inline in the main file instead of stubbed?
Possibly hausdorff-support-function-lipschitz, if the formalizer restricts to finite-dimensional Ω → ℝ and only needs L = ‖ℓ‖. It is elementary enough to be a local lemma. That said, keeping it as a theorem ... := sorry inventory item is acceptable for the soft scaffold. The Mathlib-confirmed items nonnegative-integral-zero and atomless-singleton-null should not be stubbed.

Coverage Audit

Did the original audit miss any external result the structurer listed?
No. The 17 externals listed in the structurer packet are all represented in the audit. The main theorem’s shorter dependency list is a subset of the full external inventory, not a gap.

Are there results not in the structurer's list that the audit should not have considered?
No substantive extras. The companion Mathlib probes, such as stdSimplex.zero_le, Measure.compProd, Metric.NonemptyCompacts.dist_eq, and geometric_hahn_banach_point_closed, are helper lemmas for the same externals, not new external dependencies.

Bottom Line

This is a small patch, not a storm evacuation. The not_in_mathlib inventory is basically right, and the confirmed Mathlib names mostly have the right semantics. The two blockers for green-lighting formalization are:

Patch the kernel-integration confirmed match so it covers compProd and kernel iterated integrals, not just ordinary μ.prod ν.

Replace placeholder or under-specified INVENTORY.lean stubs with faithful theorem-shaped statements.

After that, the audit can move to PASS and formalization can begin.
