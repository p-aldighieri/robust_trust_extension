...
\begin{equation}\label{eq:bayescone}
  ...
  = N_W(w)\cap\Delta(\Om),
\end{equation}
where $N_W(w)$ is the normal cone of $W$ at $w$. The messagewise Bayes cone $B(m):=B_W(w^*(m))$ used throughout is the composition of the payoff-label selector $m\mapsto w^*(m)$ with $B_W$, i.e. $B$ is the construction map $\mathrm{Belief}(\Om)\to\mathrm{Set}(\Delta(\Om))$, $p\mapsto B_W(w^*(p))$, evaluated at the message-induced prior. We write $B(m)=\mathrm{bayesConeFromPrior}(\mathrm{prior}(m))$ when the construction-map structure matters.
...
\paragraph{Assembly order of the binary capstone.} The proof is staged: (B2) TRS interval reduction $T^*=[L,R]$; (B3) endpoint-only projected image (Lemma~\ref{lem:binaryimage}); (B1) endpoint-fiber lift via Strassen-style scalar coupling on each fiber (Lemma~\ref{lem:B1}); (B4) interior calibration $P_{\beta^*}(\cdot\mid m)=m$ for $m\in(L,R)\cap M$; (B5) endpoint stationarity giving the total-balance equations (Lemma~\ref{lem:binarystat}); (B6) capstone assembly via Hall pasting plus the truthful interior bridge, then $q$-a.e. verification of Definition~\ref{def:RR} above. Each step uses only the previously established steps; in particular B1 consumes the balances proved in B5.
...
\paragraph{F4 disintegration data.} The FBNF capstone consumes the following typed disintegration data, which we name once and reuse throughout this section: (i) a measurable foliation projection $\mathrm{foliationProjection}:M\to Z$ onto a standard Borel base $Z$; (ii) a jointly Borel fiber chart $\mathrm{fiberChart}:Z\times\R\to M$, $(z,t)\mapsto\ell_z(t)$; (iii) a $\tau_M$-fiber-disintegration $\mathrm{tauFiber}:Z\to\mathcal P(M)$, $z\mapsto\tau_z$, with $\tau_M=\int_Z\tau_z\,\lambda(dz)$ and $\mathrm{supp}(\tau_z)\subseteq\ell_z([a_z,b_z])$; (iv) the B/G fiber-alignment $\tau_M$-a.e.\ equalities $B(\ell_z(t))\cap\ell_z([a_z,b_z])=\{\ell_z(L(z)),\ell_z(R(z))\}$ and $G(s)\cap\ell_z([a_z,b_z])=\argmin_{u\in[a_z,b_z]}s\cdot w^*(\ell_z(u))$. The alignment equalities are what turn fiber-local rowwise minimizers into global rowwise minimizers, i.e.\ they realize global fiber dominance in disintegrated form.
...
\paragraph{Per-class structural $\Psi$ upper bounds.} Each primitive class below proves $\Psi(y)\le0$ by first establishing a class-specific \emph{structural upper bound} of the form $\Psi(y)\le \Psi^{\text{class}}(y)$, where $\Psi^{\text{class}}$ is a class-specific integral whose nonpositivity is read directly from the geometry:
\begin{itemize}[leftmargin=*]
  \item P2$^*$: $\Psi(y)\le \alpha\int_M(\mathrm{jam}(m)-\eta)\tau(dm)$ (displacement bound plus Bayes-cone consistency).
  \item P3/G4: $\Psi(y)\le \sum_{j,\ell}(g_{j\ell}\cdot n_j - c_{j\ell}q_j)$ (finite-facet LP residual; \eqref{eq:LP} below).
  \item P4 radial: $\Psi(y)\le \int_M R(m)\tau(dm)$ where $R$ is the reflection-balance integrand of Proposition~\ref{prop:P4}.
  \item FBNF: $\Psi(y)\le \int_Z\!\!\int \Psi_z(t)\tau_z(dt)\lambda(dz)$ (fiber-disintegrated upper bound; Lemma~\ref{lem:F1}).
  \item Finite-graph FBNF: $\Psi(y)\le \sum_{e\in E}\int \Psi_e \tau_e$ (edgewise upper bound under Kirchhoff balance; Proposition~\ref{prop:graphfbnf}).
\end{itemize}
Stating these explicitly makes it visible where geometry enters the calibration: $\Psi^{\text{class}}\le0$ is a structural certificate, not an unverified $\Psi\le0$ hypothesis.
...
\paragraph{Reflection-balance integrand.} The ``antipodal $\tau$-symmetry'' driving Proposition~\ref{prop:P4} is a per-message measurable function $R:M\to\R$, the \emph{reflection-balance integrand}, defined by
\[
  R(m)
  = y(m)\cdot m - h_{B(m)}(y(m))
   + y(\sigma(m))\cdot\sigma(m) - h_{B(\sigma(m))}(y(\sigma(m))),
\]
where $\sigma:M\to M$ is the radial antipodal involution. Under the radial/equivariant primitives, $R$ satisfies the $\sigma$-antisymmetry identity $R(\sigma(m))=-R(m)$ $\tau$-a.e., so $\int_M R(m)\tau(dm)=0$. This vanishing integral is the scalar radial balance that calibrates boundary posteriors; the P4 structural bound $\Psi(y)\le\int_M R\,d\tau=0$ then closes the argument.
...
\paragraph{Explicit edge-flow LP.} Let $G=(V,E)$ be the finite embedded graph. The graph-FBNF calibration is the following edge-flow LP. The variables are edgewise scalar transports $x_e^L, x_e^R\ge 0$ for each $e\in E$, realizing the endpoint-fiber balances along $e$. For each vertex $v\in V$ with incident half-edges $\{e_1,\ldots,e_{d(v)}\}$, define signed half-edge contributions $\delta_{v,e_i}$ (positive inflow, negative outflow). The constraints are (i) per-edge endpoint balances (Lemma~\ref{lem:F3} on each $e$); (ii) Kirchhoff node balance $\sum_{i=1}^{d(v)}\delta_{v,e_i}=0$ at every $v\in V$; (iii) cross-edge dominance $\min_{\mu\in T}\ell_e(t)\cdot w^*(\mu)=\min_{\mu\in T_e}\ell_e(t)\cdot w^*(\mu)$ for $\bar\tau$-a.e.\ $(e,t)$ (the edge analogue of FBNF-7). Under these, the structural upper bound is $\Psi(y)\le\sum_{e\in E}\int\Psi_e(t)\tau_e(dt)$, with each $\Psi_e$ nonpositive by the per-edge Lemma~\ref{lem:F1} argument.
...
\paragraph{Explicit LP variables and Farkas instance.} The LP behind \eqref{eq:LP} is a concrete finite LP, not a black-box reduction. Its variables are $x_{ij}\ge0$, the flow from source cell $i$ to active label $j$. Its constraints are: (i) source balance $\sum_j x_{ij}=(1-\alpha)\tau(S_i)$ for each $i$; (ii) the aggregates $q_j,n_j$ defined as in \eqref{eq:qjnj} with the adversarial contribution $\sum_i x_{ij}s_i$ replacing $(1-\alpha)\mu_j\bar s_j$; (iii) facet feasibility $g_{j\ell}\cdot n_j\le c_{j\ell}q_j$ for each $(j,\ell)$. Infeasibility of this LP is equivalent, by conic Farkas, to existence of dual prices $y_{j\ell}\ge0$ certifying $\Psi(y)>0$; this is the Farkas instance encoded by the LP, and the threshold computation \eqref{eq:WTAthreshold} is its $K=3$ WTA instantiation.
...
\paragraph{Threshold convention (clarification).} The threshold is $D\ge2(1-\alpha)/(9\alpha)$ as displayed in \eqref{eq:WTAthreshold}. The reciprocal-form $D\ge9\alpha/(2(1-\alpha))$ is \emph{incorrect}; an earlier internal draft contained it as a transcription error and was corrected on 2026-05-21 (see correction note in \texttt{v9\_consolidated.md} \S B.7 WTA threshold normalization). The Lean theorem \texttt{Hall-WTA-reopening-threshold-D} verifies the corrected algebra. This paper displays only the corrected form everywhere.
